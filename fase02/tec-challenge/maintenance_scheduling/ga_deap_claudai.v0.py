...
class TurnScheduling:
    ...
    def setup_deap(self):
        ...
        self.toolbox.register("individual", tools.initRepeat, creator.Individual, 
                               self.toolbox.attr_employee, n=len(self.ordens))
    ...
    def select_employee(self):
        ...
        for _, ordem in self.ordens.iterrows():
            ...
            qualificacoes = ordem['qualificacao'].split('/') if ordem['qualificacao'] else []
            
            # Employees in the same work center
            centro_trab_employees = self.disponibilidade[
                self.disponibilidade['centro_trabalho'] == centro_trabalho
            ]['matricula'].tolist()
            
            # Filter qualified employees
            qualified_employees = self.disponibilidade[
                (self.disponibilidade['centro_trabalho'] == centro_trabalho) &
                (self.disponibilidade['qualificacao'].apply(
                    lambda x: not qualificacoes or any(q in x.split('/') for q in qualificacoes)))
            ]['matricula'].tolist()
            
            # Use maintenance history if no qualified employees are available
            ...
            # Default to all employees in the work center if no qualified employees
            employees_to_use = qualified_employees or centro_trab_employees
            
            # Always append a single string or an empty string
            valid_employees.append(str(random.choice(employees_to_use)) if employees_to_use else '')
    ...
    def evaluate_schedule(self, individual):
        ...
        for i, (employee, ordem) in enumerate(zip(individual, self.ordens.itertuples())):
        ...
        for i in range(len(individual)):
            if random.random() < indpb:
                valid_employees = self.disponibilidade[
                    (self.disponibilidade['centro_trabalho'] == self.ordens.iloc[i]['centro_trabalho'])
                ]['matricula'].tolist()
                individual[i] = random.choice(valid_employees or [''])
        return (individual,)  # Retorna uma tupla contendo o indivíduo mutado
    ...
    def format_schedule(self, solution):
        ...
        schedule_results = []
        for i, (employee, ordem) in enumerate(zip(solution, self.ordens.itertuples())):
...
def main():
    try:
        ...
        resultado_escalonamento = scheduler.solve(10,10)
