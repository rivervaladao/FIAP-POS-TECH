...
def split_audio(audio_path, segment_duration=30):
    ...
    for i in range(0, len(audio), segment_duration * 1000):
        ...
        chunk_path = f"{audio_path}_{start_ms//1000}.wav"
        chunk.export(chunk_path, format="wav")
        chunks.append((chunk_path, start_ms / 1000, end_ms / 1000))
    return chunks

def transcribe_audio_to_text(audio_path, text_output_path, video_url=None, with_timestamp=True, segment_duration=30):
    ...
    if file_size_mb > max_size_mb:
        print(f"Arquivo de áudio ({file_size_mb:.2f}MB) excede o limite ({max_size_mb}MB). Dividindo...")
        chunks = split_audio(audio_path, segment_duration=segment_duration)
    else:
        chunks = [(audio_path, 0, len(AudioSegment.from_wav(audio_path))/1000)]
    
    try:
        with tqdm(total=len(chunks), desc="Transcrevendo trechos de áudio") as pbar:
...
def main():
    ...
    parser.add_argument('--segment-duration', type=int, default=30, help='Duração de cada segmento de áudio em segundos')
    ...
    try:
        if source_type == 'youtube':
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                transcribe_audio_to_text(audio_path, text_output_path, video_url=input_source, with_timestamp=with_timestamp, segment_duration=segment_duration)
        else:  # file
            extract_audio_from_video(input_source, audio_path, start_time, end_time)
            transcribe_audio_to_text(audio_path, text_output_path, with_timestamp=with_timestamp, segment_duration=segment_duration)
